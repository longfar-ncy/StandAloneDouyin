...
const (
	...
	BigContent   = "Comment for big star's video"
	MidContent   = "Comment for mid star's video"
	SmallContent = "Comment for small star's video"
)
...
func main() {
	...
	go CommentForSmallStars()
	go CommentForMidStars()
	go CommentForBigStars()
	for i := 0; i < 3; i++ {
		<-ch
	}
}
...
func CommentForSmallStars() {
	for _, u := range SmallStars {
		for _, id := range u.VidList {
			n := rand.Intn(NumSmallComment/5) - NumSmallComment/10 + NumSmallComment
			idxs := Sample(NumNormal, n)
			for _, i := range idxs {
				DoComment(TotalUsers[i].Token, id, SmallContent)
			}
		}
	}
...
func CommentForMidStars() {
	for _, u := range MidStars {
		for _, id := range u.VidList {
			n := rand.Intn(NumMidComment/5) - NumMidComment/10 + NumMidComment
			idxs := Sample(NumNormal+NumSmallStar, n)
			for _, i := range idxs {
				DoComment(TotalUsers[i].Token, id, MidContent)
			}
		}
	}
...
func CommentForBigStars() {
	for _, u := range BigStars {
		for _, id := range u.VidList {
			n := rand.Intn(NumBigComment/5) - NumBigComment/10 + NumBigComment
			idxs := Sample(NumNormal+NumSmallStar+NumMidStar, n)
			for _, i := range idxs {
				DoComment(TotalUsers[i].Token, id, BigContent)
			}
		}
	}
...
func DoComment(token string, vid int64, content string) {
	q := map[string]string{
		"token":        token,
